		...
		>
			<div className="w-100" style={{ maxWidth: "1100px", margin: "0 auto" }}>
				...
				<div className="mb-5">
					...
					>
						{highlightText(t("quickstart.title"), searchTerm)}
					</div>
				</div>
				...
				{searchTerm && (
					...
					>
						{/* 🔍 {t("quickstart.searchResults")} "{searchTerm}": 
						<strong> {filteredArticles.length} </strong> 
						{filteredArticles.length === 1 ? t("quickstart.result") : t("quickstart.results")} */}
					</div>
				)}
				...
				{filteredArticles.length > 0 ? (
					...
					>
						...
						>
							{t("quickstart.noResults", { searchTerm })}
						</p>
					</div>
				)}
			</div>
