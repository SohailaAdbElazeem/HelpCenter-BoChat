		...
		>
			<div className="w-100" style={{ maxWidth: "1050px", margin: "0 auto" }}>
				...
				<div className="mb-5">
					...
					>
						{highlightText(t("subscriptions.title"), searchTerm)}
					</div>
				</div>
				...
				{searchTerm && (
					...
					>
						{/* 🔍 {t("subscriptions.searchResults")} "{searchTerm}": 
						<strong> {filteredArticles.length} </strong> 
						{filteredArticles.length === 1 ? t("subscriptions.result") : t("subscriptions.results")} */}
					</div>
				)}
				...
				{filteredArticles.length > 0 ? (
						...
						{filteredArticles.map((article, index) => (
							<div key={article.key} className="col-12 col-md-6 d-flex">
								<div
									...
									onClick={() => {
										// يمكن إضافة وظيفة عند النقر على المقال
										console.log("Subscription article clicked:", article.title);
									}}
								>
									{/* Red Dot */}
									<div
					...
					>
						<p style={{ fontSize: "18px", color: "#666", marginBottom: "20px" }}>
							{t("subscriptions.noResults", { searchTerm })}
						</p>
					</div>
				)}
			</div>
